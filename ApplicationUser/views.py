...
def signup(request):
    ...
    try:
        ...
        phone = data["phone"]
        if first_name != "" and last_name != "" and email != "" and password != "" and password_confirmation != "" and phone != "":
            if password == password_confirmation:
                user = User.objects.create(username=first_name + ' ' + last_name, first_name=first_name,
                ...
                if user is not None:
                    ...
                    if tourist is not None:
                        ...
                        response["message"] = "Something went wrong. try again later. if the issue comes back please contact us."
                else:
                    response["message"] = "Something went wrong. recheck the filled data. fill all the fields and try again. if the issue comes back please contact us."
            else:
...
def sign_in(request):
    ...
    try:
        ...
        if email != "" and password != "":
            ...
            response["message"] = "Please check all the field filled in the form. make sure every field is filled and try again. if the issue persists contact us."
    except Exception as e:
        response["message"] = "Please check all the field filled in the form. make sure every field is filled and try again. if the issue persists contact us."
        print(e)
    return Response(response)
    ...
    try:
        ...
        if tourInquiry is not None:
            ...
            for transportType in transportTypes:
                try:
                    transportTypeObject = TransportType.objects.get(
                        id=transportType)
            ...
            # 		print(e)
            notifyAdmin(
                "New Inquiry", "New inquiry has been made. please check it out!", data)
            response["status"] = "ok"
    except Exception as e:
        print(e)
        pass
...
def get_tourinquiry(request):
    ...
    response["AvailableLanguage"] = [{"id": item.id, "language": item.language}
                                     for item in tourinquiry.availablelanguage_set.all()]
    response["TransportType"] = [{"id": item.id, "type": item.type}
                                 for item in tourinquiry.transporttype_set.all()]
    ...
    response["Currency"] = [{"id": item.id, "currency": item.currency}
                            for item in tourinquiry.currency_set.all()]
...
def get_tourinquiryhasagecategory(request):
    ...
    response["AgeCategory"] = [{"id": item.id, "category": item.category, "min_age": item.min_age,
                                "max_age": item.max_age} for item in tourinquiryhasagecategory.agecategory_set.all()]
    response["TourInquiry"] = [{"id": item.id, "full_name": item.full_name, "arrival": item.arrival, "departure": item.departure, "arrival_airport": item.arrival_airport, "departure_airport": item.departure_airport, "nationality": item.nationality, "duration_of_stay": item.duration_of_stay, "need_visa_assistance": item.need_visa_assistance,
                                "need_travel_insurance": item.need_travel_insurance, "guide_language": item.guide_language, "transport_type": item.transport_type, "age_categories": item.age_categories, "places": item.places, "estimated_budget": item.estimated_budget, "currency": item.currency} for item in tourinquiryhasagecategory.tourinquiry_set.all()]
...
def update_tourinquiryhasagecategory(request):
    ...
    try:
        tourinquiryhasagecategory = TourInquiryHasAgeCategory.objects.get(
            id=id)
            ...
            for ageRange in selectedAgeRange:
                try:
                    ageCategory = AgeCategory.objects.get(
                        category=ageRange["category"])
                    print(ageCategory)
                    totalPrice += int(ageRange["numberOfPeople"]
                                      ) * ageCategory.price
...
def edit_tourpackage(request):
    ...
    try:
        ...
        if tourPackage is not None:
            ...
            totalPrice = 0.0
            for ageRange in selectedAgeRange:
                ageCategory = tourPackage.tourpackagehasagecategory_set.filter(
                    agecategory_category=ageRange["category"])
